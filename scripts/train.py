...
if __name__ == "__main__":
    ...
    # Define label names to class_ids
    id2label = {0: 'Negativo',1: 'Positivo'}
    label2id = {v: k for k, v in id2label.items()}
    # download model from model hub
    model = AutoModelForSequenceClassification.from_pretrained(args.model_name, num_labels=2, id2label=id2label,
                                                               label2id=label2id)
    tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    # Setting eval steps
    #evals_per_epoch = 3
    #eval_steps = (len(train_dataset)//args.train_batch_size)//evals_per_epoch
    #print("Evaluation steps: ", eval_steps)
    # Setting logging steps
    logs_per_epoch = 10
    log_steps = (len(train_dataset)//args.train_batch_size)//logs_per_epoch
    #print("Logging steps: ", log_steps)
